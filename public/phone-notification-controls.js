(function(){
  ...
  async function testAlarm(){if(working)return;working=true;update();try{await reminders.test();flash('🔔 Sesli deneme bildirimi gönderildi. Telefonun bildirim sesini kontrol edin.')}catch(error){flash(error.message)}finally{working=false;update()}}
  function init(){
    ...
    const test=document.createElement('button');test.id='mobileNotifyTest';test.type='button';test.textContent='🔊 SESLİ BİLDİRİMİ DENE';test.onclick=testAlarm;status.insertAdjacentElement('afterend',test);update();
  }
