(function(){
  ...
  function update(){
    ...
    else if(!reminders.supported()){btn.textContent=onPlanner()?'🔔 Ajanda Alarmı':'🔔 Bildirim';text='Telefon bildirimi için güncel Chrome veya Safari kullanın.'}
    ...
    else if(reminders.enabled()&&reminders.status.connected){btn.textContent=onPlanner()?'✅ Ajanda Alarmı Açık':'✅ Bildirim Açık';text='✅ AJANDA ALARMI AÇIK — AJANDADAKİ SAATTE TELEFONA BİLDİRİM GÖNDERİLİR';mode='ok'}
    else if(reminders.status.error){btn.textContent=onPlanner()?'🔔 Ajanda Alarmını Aç':'🔔 Bildirimi Aç';text=reminders.status.error}
    else if(!reminders.enabled()){btn.textContent=onPlanner()?'🔕 Ajanda Alarmını Aç':'🔕 Bildirim Kapalı';text='🔕 TELEFON BİLDİRİMLERİ KAPALI — AÇMAK İÇİN BAS';mode='bad'}
    else{btn.textContent=onPlanner()?'🔔 Ajanda Alarmını Aç':'🔔 Bildirimi Aç';text='🔔 KİLİT EKRANINDA UYARI ALMAK İÇİN BİLDİRİMLERİ AÇIN'}
