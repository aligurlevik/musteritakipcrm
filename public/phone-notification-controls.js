(function(){
  ...
  function update(){
    ...
    else if(!reminders.supported()){btn.textContent=onPlanner()?'🔔 AJANDA ALARMI':'🔔 Bildirim';text='Telefon bildirimi için güncel Chrome veya Safari kullanın.'}
    ...
    else if(reminders.enabled()&&reminders.status.connected){btn.textContent=onPlanner()?'✅ AJANDA ALARMI AÇIK':'✅ Bildirim Açık';text='✅ AJANDA ALARMI AÇIK — AJANDADAKİ SAATTE TELEFONA BİLDİRİM GÖNDERİLİR';mode='ok'}
    else if(reminders.status.error){btn.textContent=onPlanner()?'🔔 AJANDA ALARMINI AÇ':'🔔 Bildirimi Aç';text=reminders.status.error}
    else if(!reminders.enabled()){btn.textContent=onPlanner()?'🔔 AJANDA ALARMINI AÇ':'🔕 Bildirim Kapalı';text='🔕 TELEFON BİLDİRİMLERİ KAPALI — AÇMAK İÇİN BAS';mode='bad'}
    else{btn.textContent=onPlanner()?'🔔 AJANDA ALARMINI AÇ':'🔔 Bildirimi Aç';text='🔔 KİLİT EKRANINDA UYARI ALMAK İÇİN BİLDİRİMLERİ AÇIN'}
  ...
  function init(){
    ...
    const test=document.createElement('button');test.id='mobileNotifyTest';test.type='button';test.textContent=onPlanner()?'🔊 AJANDA ALARM TESTİ':'🔊 ANDROID BİLDİRİM TESTİ';test.onclick=testAlarm;status.insertAdjacentElement('afterend',test);
    if(!onPlanner()){const pair=document.createElement('button');pair.id='nativeAlarmPair';pair.type='button';pair.textContent='📱 GERÇEK ALARM UYGULAMASINI BAĞLA';pair.onclick=pairNativeAlarm;test.insertAdjacentElement('afterend',pair)}
    update();
  }
