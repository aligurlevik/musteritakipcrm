(function(){
  ...
  async function testAlarm(){if(working)return;working=true;update();try{await reminders.test();flash('🔔 2 aşamalı test gönderildi: önce 1/2 yerel, sonra 2/2 sunucu bildirimi gelmeli. Hiçbiri görünmezse Android tarayıcı bildirim ayarı engelliyor.')}catch(error){flash(error.message)}finally{working=false;update()}}
  function init(){
