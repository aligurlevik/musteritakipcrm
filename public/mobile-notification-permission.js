(function(){
  ...
  function flash(text){
    ...
    el.textContent=text;el.style.display='block';clearTimeout(window.__notifyMsgTimer);window.__notifyMsgTimer=setTimeout(()=>el.style.display='none',5000)
  }
  function ensureStatus(){
    let el=document.getElementById('mobileNotifyStatus');
    if(el)return el;
    el=document.createElement('div');
    el.id='mobileNotifyStatus';
  ...
  function updateState(btn){
    ...
    if(isiOS()&&!isStandalone()){
      ...
      setStatus('📱 BİLDİRİM İÇİN: ANA EKRANA EKLE','warn');
      return;
    }
    if(Notification.permission==='granted'){
      btn.textContent='✅ Bildirimler Açık';
      setStatus('✅ BİLDİRİMLER AÇIK','ok');
    }else if(Notification.permission==='denied'){
      btn.textContent='❌ Bildirim Kapalı';
      setStatus('❌ BİLDİRİMLER KAPALI','bad');
    }else{
      btn.textContent='🔔 Bildirimlere İzin Ver';
      setStatus('🔔 BİLDİRİM İZNİ BEKLİYOR','info');
    }
  }
  async function ask(){
    ...
    if(!('Notification' in window)){
      updateState(btn);
      if(isiOS()&&!isStandalone())flash('iPhone’da Safari → Paylaş → Ana Ekrana Ekle yapın. Sonra ajandayı ana ekrandaki simgeden açın.');
      else flash('Bu tarayıcı bildirim iznini desteklemiyor.');
      return;
    }
    if(isiOS()&&!isStandalone()){
      updateState(btn);
      flash('Önce Safari → Paylaş → Ana Ekrana Ekle yapın. Sonra ajandayı ana ekrandaki simgeden açıp bu düğmeye tekrar basın.');
      return;
    }
    try{
      const p=await Notification.requestPermission();updateState(btn);
      if(p==='granted'){
  ...
  function init(){
    ...
    if(!btn){
      ...
      btn=document.createElement('button');btn.id='mobileNotifyBtn';btn.type='button';btn.onclick=ask;host.insertBefore(btn,host.firstChild);
    }
    updateState(btn);
  }
