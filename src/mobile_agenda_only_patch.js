...
<style>
  @media (max-width:820px){
    body.mobile-agenda-only .app{
      display:block!important;
      min-height:100vh!important;
    }
    body.mobile-agenda-only .side{
      display:none!important;
    }
    body.mobile-agenda-only .main{
      padding:10px!important;
      width:100%!important;
      max-width:none!important;
    }
    body.mobile-agenda-only .top{
      display:none!important;
    }
    body.mobile-agenda-only .section{
      display:none!important;
    }
    body.mobile-agenda-only #agenda{
      display:block!important;
      width:100%!important;
      margin:0!important;
    }
    body.mobile-agenda-only #agenda .agenda-overview{
...
(function(){
  ...
  function activateMobileAgenda(role){
  ...
  fetch('/api/session').then(function(r){return r.ok?r.json():null}).then(function(session){
    if(session)activateMobileAgenda(session.role);
  }).catch(function(){});

  window.addEventListener('resize',function(){
    if(!isPhone())document.body.classList.remove('mobile-agenda-only');
    else fetch('/api/session').then(function(r){return r.ok?r.json():null}).then(function(session){if(session)activateMobileAgenda(session.role)}).catch(function(){});
  });
})();
</script>`;
...
export default {
  async fetch(request,env,ctx){
    ...
    if(request.method==='GET'&&response.status===200&&(url.pathname==='/'||url.pathname==='/index.html')&&contentType.includes('text/html')){
      ...
      if(!html.includes('mobile-agenda-only'))html=html.replace('</body>',mobileAgendaOnlyPatch+'\n</body>');
