...
async function api(request, env) {
  ...
  if(path==='/api/agenda/active'&&request.method==='GET'){return json((await env.DB.prepare("SELECT * FROM agenda_entries WHERE COALESCE(source_type,'manual')='manual' AND COALESCE(entry_status,'Yapılacak')<>'Yapıldı' ORDER BY entry_date,sort_order,id").all()).results)}
