...
async function ensureSchema(env) {
  ...
  await ensureColumn(env,'agenda_entries','list_type',"TEXT DEFAULT 'Yapılacak'");
...
async function api(request, env) {
  ...
  if(path==='/api/agenda'&&request.method==='POST'){const b=await body(request);if(!b.entry_date||!String(b.note||'').trim())return json({error:'Tarih ve ajanda notu zorunlu.'},400);const listType=b.list_type==='Yapılan'?'Yapılan':'Yapılacak';const last=await env.DB.prepare('SELECT COALESCE(MAX(sort_order),0) n FROM agenda_entries WHERE entry_date=? AND list_type=?').bind(b.entry_date,listType).first();const remindAt=b.remind_at||'';const created=await env.DB.prepare('INSERT INTO agenda_entries(entry_date,sort_order,note,remind_at,reminder_status,list_type) VALUES(?,?,?,?,?,?)').bind(b.entry_date,Number(last?.n||0)+1,String(b.note).trim(),remindAt,remindAt?'Açık':'',listType).run();return json({ok:true,id:created.meta.last_row_id},201)}
  const agendaItem=path.match(/^\/api\/agenda\/(\d+)$/);
  if(agendaItem&&request.method==='PUT'){const id=Number(agendaItem[1]),b=await body(request);if(!b.entry_date||!String(b.note||'').trim())return json({error:'Tarih ve ajanda notu zorunlu.'},400);const remindAt=b.remind_at||'',listType=b.list_type==='Yapılan'?'Yapılan':'Yapılacak';const ex=await env.DB.prepare('SELECT id FROM agenda_entries WHERE id=?').bind(id).first();if(!ex)return json({error:'Ajanda notu bulunamadı.'},404);await env.DB.prepare('UPDATE agenda_entries SET entry_date=?,note=?,remind_at=?,reminder_status=?,list_type=? WHERE id=?').bind(b.entry_date,String(b.note).trim(),remindAt,remindAt?'Açık':'',listType,id).run();return json({ok:true})}
