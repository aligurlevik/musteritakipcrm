...
async function ensureSchema(env) {
  ...
  await env.DB.prepare("UPDATE agenda_entries SET source_type='graphic_job' WHERE COALESCE(source_type,'manual')='manual' AND EXISTS (SELECT 1 FROM graphic_jobs g WHERE agenda_entries.note LIKE g.customer_name||' — '||g.job_no||'%')").run();
...
async function api(request, env) {
  ...
  if(path==='/api/agenda/reminders'&&request.method==='GET'){return json((await env.DB.prepare("SELECT * FROM agenda_entries WHERE COALESCE(source_type,'manual')='manual' AND reminder_status='Açık' AND remind_at<>'' ORDER BY remind_at").all()).results)}
  if(path==='/api/agenda/rollover'&&request.method==='POST'){
    ...
    const moved=await env.DB.prepare("UPDATE agenda_entries SET entry_date=?, remind_at=CASE WHEN COALESCE(remind_at,'')<>'' THEN ?||substr(remind_at,11) ELSE COALESCE(remind_at,'') END, reminder_status=CASE WHEN COALESCE(remind_at,'')<>'' THEN 'Açık' ELSE COALESCE(reminder_status,'') END WHERE COALESCE(source_type,'manual')='manual' AND entry_date<? AND COALESCE(entry_status,'Yapılacak')<>'Yapıldı'").bind(today,today,today).run();
    return json({ok:true,moved:Number(moved.meta?.changes||0)});
  }
  if(path==='/api/agenda/day'&&request.method==='GET'){const date=url.searchParams.get('date')||new Date().toISOString().slice(0,10);return json((await env.DB.prepare("SELECT * FROM agenda_entries WHERE COALESCE(source_type,'manual')='manual' AND entry_date=? ORDER BY sort_order,id").bind(date).all()).results)}
  if(path==='/api/agenda/completed'&&request.method==='GET'){const date=url.searchParams.get('date')||new Date().toISOString().slice(0,10);return json((await env.DB.prepare("SELECT * FROM agenda_entries WHERE COALESCE(source_type,'manual')='manual' AND entry_status='Yapıldı' AND completed_date=? ORDER BY id DESC").bind(date).all()).results)}
  if(path==='/api/agenda/active'&&request.method==='GET'){const from=url.searchParams.get('from')||new Date().toISOString().slice(0,10);return json((await env.DB.prepare("SELECT * FROM agenda_entries WHERE COALESCE(source_type,'manual')='manual' AND entry_date>=? AND COALESCE(entry_status,'Yapılacak')<>'Yapıldı' ORDER BY entry_date,sort_order,id").bind(from).all()).results)}
  if(path==='/api/agenda'&&request.method==='GET'){const month=url.searchParams.get('month')||new Date().toISOString().slice(0,7);return json((await env.DB.prepare("SELECT * FROM agenda_entries WHERE COALESCE(source_type,'manual')='manual' AND entry_date LIKE ? ORDER BY entry_date,sort_order,id").bind(month+'%').all()).results)}
  if(path==='/api/agenda'&&request.method==='POST'){const b=await body(request);if(!b.entry_date||!String(b.note||'').trim())return json({error:'Tarih ve ajanda notu zorunlu.'},400);const imageData=validAgendaImage(b.image_data);if(imageData===null)return json({error:'Resim geçersiz veya çok büyük.'},400);const last=await env.DB.prepare("SELECT COALESCE(MAX(sort_order),0) n FROM agenda_entries WHERE COALESCE(source_type,'manual')='manual' AND entry_date=?").bind(b.entry_date).first();const remindAt=b.remind_at||'';const created=await env.DB.prepare("INSERT INTO agenda_entries(entry_date,sort_order,note,remind_at,reminder_status,image_data,source_type) VALUES(?,?,?,?,?,?,'manual')").bind(b.entry_date,Number(last?.n||0)+1,String(b.note).trim(),remindAt,remindAt?'Açık':'',imageData).run();return json({ok:true,id:created.meta.last_row_id},201)}
