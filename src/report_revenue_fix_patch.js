...
<style>
  ...
  #reports .revenue-compare-card b{
...
export default {
  async fetch(request, env, ctx) {
    ...
    if(request.method==='GET'&&response.status===200&&(url.pathname==='/'||url.pathname==='/index.html')&&contentType.includes('text/html')){
      ...
      if(!html.includes('/* Ciro tutarlarini daha belirgin goster. */'))html=html.replace('</body>',ciroReadabilityCss+'\n</body>');
